...
/**
 * The State Holder for the theme settings UI.
 *
 * This ViewModel follows the MVI (Model-View-Intent) pattern:
 * 1. **State**: Exposes [themeConfig] as a [StateFlow] derived from the persistence layer.
 * 2. **Intents**: Provides a series of `set...` functions that translate UI interactions into 
 *    asynchronous repository operations.
 *
 * It is lifecycle-aware and uses [SharingStarted.WhileSubscribed] to ensure that 
 * persistence flows are only active when the settings screen is actually visible.
 */
@HiltViewModel
